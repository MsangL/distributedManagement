...
function callD(typ::Int,Tli::Number,Wshare)
	...
	open("Coube","w") do io
       ...
       production= Array{Float64}(undef, (T,u))
       ...
       cv=Pvc.*cv 
        ...
        while iter<limI && enN>=Wshare
                for i in 1:n
                        ...
                        for t in 1:T 
                                for l in 1:u 
                                        Stock[t,l]+=round(sum(E[b,t,l]/dist for b in 1:B),digits=2) # the total storage level
                                        for b in 1:B 
                                                if q[b,t,l]>=0 
...
function callF()
                                                        # typ determine the type of reparatition keys if type==1 enedis, 
        Tli=50.0
